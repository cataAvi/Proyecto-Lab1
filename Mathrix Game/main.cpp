...
//Ejercicio para rendir:

Apellidos A - L (inclusive):
- Al terminar una partida, mostrar al jugador el nombre y puntaje y
también la cantidad de sumas, restas, multiplicaciones y divisiones (cociente y resto) realizadas
durante el juego.

Apellidos M (inclusive) - Z (inclusive) : Al ingresar las coordenadas de matriz 0,0 se pierde
una pila pero se aleatoriza nuevamente toda la matriz.

Revisar tema division por 0
...
void jugar (char nombrePuntajeMax [], int &puntajeMax)
{
    ...
    while (opFinalJuego == 'N')
    {
        ...
        char calcFav [45];
        //Parcial
        int cantOperaciones [opDisp]; //(+,-,*,/,%) --> 0,1,2,3
        ...
        while (pilas > 0)
        {
            ...
            if (validacionEspacios(matrizSombra, f1, c1, f2, c2, f3, c3) && validacionRango(f1, c1, f2, c2, f3, c3, x, y))
            {
                ...
                if (validacionOperacion (matrizJugar, f1, c1, f2, c2, f3, c3, operador))
                {
                    ...
                    calOp(cantOperaciones,operador);
                }
                else
                {
                    pilas --;
            ...
            {
                if (!(validacionRango(f1, c1, f2, c2, f3, c3, x, y))){
                    if (f1 == -1 && c1 == -1){
                        cargarMatriz(matrizJugar, x, y);
                    }
                }
        ...
        rlutil::cls();
        divGameOver();
        cout << endl << "Puntuacion final de " << vnombre << ": " << puntaje;
        cout << endl;
        rlutil::setColor(rlutil::LIGHTMAGENTA);
        ...
        //-----------------------------------------------------------------------------
        cout << endl << "Cantidad de sumas realizadas: " << cantOperaciones[0];
        cout << endl << "Cantidad de restas realizadas: " << cantOperaciones[1];
        cout << endl << "Cantidad de multiplicaciones realizadas: " << cantOperaciones[2];
        cout << endl << "Cantidad de divisiones realizadas: " << cantOperaciones[3];
        cout << endl;
        //---------------------------------------------------------------------------------
...
rlutil::cls();
    ...
    rlutil::setColor(rlutil::BLUE);
    cout << endl << endl << "https://tinyurl.com/instruccionesmathrixgame";
    rlutil::setColor(rlutil::LIGHTMAGENTA);
